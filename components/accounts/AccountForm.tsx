...
import type { FamilyMember, AccountType, Currency } from '@/lib/types';
...
export default function AccountForm({ members, onSubmit, onCancel }: AccountFormProps) {
  const [loading, setLoading] = useState(false);
  const [form, setForm] = useState({
    family_member_id: members[0]?.id ?? '',
    name: '',
    provider: '',
    account_type: 'isa' as AccountType,
  ...
  const set = (key: string, value: unknown) => {
    setForm((prev) => {
      ...
      // Auto-set liability flag based on type
      if (key === 'account_type') {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          placeholder="e.g. FTSE Global All Cap index fund"
        />
      </div>
