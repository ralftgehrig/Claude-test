...
import { Plus, Wallet, RefreshCw, ChevronDown, ChevronUp } from 'lucide-react';
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {accounts.length} accounts · {formatCurrency(totalGBP)} net
          </p>
        </div>
      ...
      {!isLoading && accounts.length === 0 && (
        <EmptyState
          ...
          description="Add your first financial account — ISA, pension, bank account, crypto, property, or anything else."
          action={
            <button className="btn-primary" onClick={() => setShowAddAccount(true)}>
        ...
        return (
          <div key={member.id} className="card">
            <button
              ...
              onClick={() => setExpandedMember(isExpanded && expandedMember === member.id ? null : member.id)}
            >
              ...
              <div className="flex items-center gap-3">
                <p className={`text-sm font-bold ${memberTotal >= 0 ? 'text-gray-900' : 'text-red-500'}`}>
                    ...
                    <div key={account.id} className="flex items-center gap-3 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">
                          {account.provider}
                          {snap && ` · Updated ${formatDate(snap.snapshot_date, 'd MMM yyyy')}`}
                          {!snap && ' · No balance recorded'}
                        </p>
                      </div>
                      ...
                      <button
                        className="btn-ghost text-xs flex-shrink-0 ml-1"
      ...
      <Modal
        ...
        title={`Update balance — ${updatingAccount?.name}`}
      >
        {updatingAccount && (
          <BalanceUpdateForm
